...
let material="SUS201";
let items=[];
const e={mat:$("materialOptions"),w:$("widthInput"),h:$("heightInput"),q:$("qtyInput"),minus:$("minusBtn"),plus:$("plusBtn"),add:$("addWindow"),tbody:document.querySelector("#windowTable tbody"),empty:$("emptyText"),curP:$("currentPyeong"),curA:$("currentAmount"),totP:$("totalPyeong"),totA:$("totalAmount"),copy:$("copyBtn"),order:$("orderBtn")};
...
function calc201(w,h){
  ...
  const widthOptionCm=Math.max(90,Math.floor(widthCm/10)*10);
  ...
  const boundaryWidthOptionCm=Math.max(90,Math.floor(boundaryWidthCm/10)*10);
...
function current(){
  ...
  e.curP.textContent=r.valid?r.basis:"규격을 입력하세요";
...
function renderAll(){
  ...
  items.forEach((it,i)=>{const r=calc(it.w,it.h,it.material);ta+=r.a;const tr=document.createElement("tr");tr.innerHTML=`<td>${i+1}</td><td>${it.material}</td><td>${it.w}</td><td>${it.h}</td><td>${r.basis}</td><td>${won(r.a)}</td><td><button class="del-btn" type="button">삭제</button></td>`;tr.querySelector("button").onclick=()=>{items.splice(i,1);renderAll()};e.tbody.appendChild(tr)});
  e.empty.style.display=items.length?"none":"flex";e.totP.textContent=items.length+" 개";e.totA.textContent=won(ta);
}
function text(){
  let ta=0;let lines=["강동자바라 스텐 방범창 견적","","※ 온라인팀 최종가격 기준",""];
  items.forEach((it,i)=>{const r=calc(it.w,it.h,it.material);ta+=r.a;lines.push(`No.${i+1} ${it.material} ${it.w} × ${it.h}mm`,`${r.basis} / ${won(r.a)}`,r.detail||"","")});
  lines.push("----------------",`총 수량: ${items.length}개`,`총 예상금액: ${won(ta)}`,"","※ SUS201: 2200mm 이하는 스마트스토어 옵션가, 초과 시 대형 맞춤 계산","※ SUS304: 온라인팀 표 적용 수량 × 1,000원","※ 가로·세로 입력범위: 1~3000mm","문의: 010-7595-0484","네이버 톡톡: https://talk.naver.com/ct/w4a85f?frm=psf");return lines.join("\n")
}
